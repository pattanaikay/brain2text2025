...
class BITModel(nn.Module):
    ...
    def forward(self, neural_data, labels=None, session_id=None, return_contrastive=True):
        ...
        # Neuro-MoE returns (fused_features, auxiliary_moe_loss)
        neural_tokens, aux_moe_loss = self.neural_encoder(neural_data, session_id=session_id) 
        ...
        if labels is not None:
            ...
            # NeuroMoE Total Loss: CE + Contrastive + alpha * Aux_MoE_Loss
            # alpha is typically small (e.g., 1e-4) as per EEGMoE paper
            loss = ce_loss + contrastive_loss + (1e-4 * aux_moe_loss)
            return loss, ce_loss, contrastive_loss, aux_moe_loss
    ...
    def generate(self, neural_data, session_id=None, max_new_tokens=100, top_p=0.9, temperature=0.7):
        self.eval()
        with torch.no_grad():
            neural_tokens, _ = self.neural_encoder(neural_data, session_id=session_id)
            projected_embeds = self.projector(neural_tokens)
