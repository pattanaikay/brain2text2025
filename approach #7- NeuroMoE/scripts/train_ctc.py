    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for batch in pbar:
            ...
            with torch.autocast(device_type='cuda', dtype=compute_dtype, enabled=args.use_amp):
                logits, aux_moe_loss = model(neural_data, session_id=session_id)
                logits = logits.permute(1, 0, 2)
                log_probs = nn.functional.log_softmax(logits, dim=2)
                
                input_lengths = torch.full((neural_data.size(0),), logits.size(0), dtype=torch.long).to(device)
                ctc_loss = ctc_loss_fn(log_probs, labels, input_lengths, target_lengths)
                
                # Total loss with MoE regularization
                loss = ctc_loss + (1e-4 * aux_moe_loss)
            ...
            pbar.set_postfix({'loss': f"{loss.item():.4f}", 'ctc': f"{ctc_loss.item():.4f}", 'moe': f"{aux_moe_loss.item():.4f}"})
