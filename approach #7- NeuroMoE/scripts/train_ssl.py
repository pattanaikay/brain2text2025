    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for batch in pbar:
            ...
            with torch.autocast(device_type='cuda', dtype=compute_dtype):
                encoded, aux_moe_loss = model(masked_data, session_id=session_id)
                ...
                mse_loss = nn.MSELoss()(reconstructed, target_data)
                loss = mse_loss + (1e-4 * aux_moe_loss)
            ...
            pbar.set_postfix({'loss': f"{loss.item():.4f}", 'mse': f"{mse_loss.item():.4f}", 'moe': f"{aux_moe_loss.item():.4f}"})
