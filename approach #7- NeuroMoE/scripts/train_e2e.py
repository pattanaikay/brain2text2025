    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for batch in pbar:
            ...
            with torch.autocast(device_type='cuda', dtype=compute_dtype):
                loss, ce_loss, contrastive_loss, aux_moe_loss = model(neural_data, labels=labels, session_id=session_id)
            ...
            total_moe += aux_moe_loss.item()
            
            pbar.set_postfix({
                'loss': f"{loss.item():.4f}",
                'ce': f"{ce_loss.item():.4f}",
                'cntr': f"{contrastive_loss.item():.4f}",
                'moe': f"{aux_moe_loss.item():.4f}"
            })

        avg_loss = total_loss / len(train_loader)
        history['train_loss'].append(avg_loss)
        logger.info(f"Epoch {epoch} Avg Loss: {avg_loss:.4f} (CE: {total_ce/len(train_loader):.4f}, Cntr: {total_contrastive/len(train_loader):.4f}, MoE: {total_moe/len(train_loader):.4f})")
